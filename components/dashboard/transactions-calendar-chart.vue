...
function mapDayInformation(perDayInfo: PerDayInformation[]): DayInfo[] {
  ...
  min = perDayInfo.reduce(
    (min, curr) => (curr.transactions < min ? curr.transactions : min),
    min,
  );
  let max = perDayInfo[0].transactions ?? 0;
  max = perDayInfo.reduce(
    (max, curr) => (curr.transactions > max ? curr.transactions : max),
    max,
  );
  ...
  const dayInformation: DayInfo[] = balancePerDay.map((dayInfo) => {
    return {
...
const calendarData = {
  ...getCalendarMetadata(),
  dayInfo: mapDayInformation(balancePerDay),
};
</script>
<template>
  <div class="max-w-56 min-w-52 border border-stone-800 p-4 rounded-lg">
    ...
    <div class="grid grid-cols-7 gap-2 justify-items-center">
      <div
        class="w-4 h-4 rounded bg-stone-800"
        v-for="_ in calendarData.firstDayIdx"
      />

      <TooltipProvider
        v-for="day in calendarData.dayInfo"
        :delay-duration="200"
      ...
      >
        <Tooltip>
          ...
          <TooltipContent>
            <p>
              {{ day.transactions }} transactions on {{ day.shortDateFormat }}
            </p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
